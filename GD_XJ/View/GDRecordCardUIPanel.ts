...
@ccclass
export default class GDRecordCardUIPanel extends UIBase 
{
    @property(cc.Sprite)
    ...
    private mGroup       : GDRecordHandCardGroup  = null;
    ...
    public IsCurSeries()
    {
        ...
        if ( this.mCardInfo.Value == ( GameDefine.MaskValue & RecordGuanDanScenes.Instance.CurSeries ) )
        {
            return true;
        }
        return false;
    ...
    {
        this.mIsSelected ? RecordGuanDanScenes.Instance.AddSelectedCard( this ) : RecordGuanDanScenes.Instance.RemoveSelectedCard( this );
    }
    ...
    private SetCardImage()
    {
        let SmallImage = "";
        let BigImage   = "";
        if ( this.mCardInfo.Card == RecordGuanDanScenes.Instance.CurSeries )
        {
        ...
        if(this.CrownRImage == null || this.CrownYImage == null) return;
        this.CrownRImage.node.active = false;
        this.CrownYImage.node.active = false;
        if ( this.mCardInfo.Card ==  RecordGuanDanScenes.Instance.CurSeries ) //  百搭
        {
            this.CrownRImage.node.active = true;
        }else if ( this.mCardInfo.Card == ( RecordGuanDanScenes.Instance.CurSeries & GameDefine.MaskValue) && this.mCardInfo.Card < 0X40 ) //  级牌
        {
    ...
    //设置进贡还贡标志
    public SetGiftCardFlag( Index : number,State : boolean )
    {
        if ( Index < 2 )
        {
