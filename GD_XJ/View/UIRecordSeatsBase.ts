...
import UIRecordRoleInfo from "./UIRecordRoleInfo";


const {ccclass, property} = cc._decorator;

@ccclass
export default class UIRecordSeatsBase extends UIBase
{
    ...
    @property(UIRecordRoleInfo)
    RoleInfo        : UIRecordRoleInfo            = null;
    @property(cc.Node)
    ...
    private mGiftCardList           : GDRecordCardUIPanel[]   = [];             //贡牌
    ...
    private InitRiver()
    {
        ...
        for ( let i = 1; i <= 10; i++ )
        {
            let Path = GameDefine.PrefabPath + "GuanDanRecordHandCardMiddle";
    ...
    //获取玩家信息
    public GetRoleInfo() : UIRecordRoleInfo
    {
    ...
    public InitHandCardPosition( i : number , Num : number , CardPanel : GDRecordCardUIPanel )
    {
    }
    ...
    public SetInitialLayout( Deal : boolean )
    {
        ...
        //排序
        RecordGuanDanScenes.Instance.SortCardList( this.mGamePlayer.HandCardList );
        //删除所有手牌实例
        this.ClearHandCards();
        //删除所有组
        this.ClearGroup();
        //创建手牌(掼蛋中仅有主玩家需要手牌列表)
        this.InitialAllHandCards( Deal );
        //重置Group
        this.ResetHandCardGroup( this.mGamePlayer.HandCardList );
        //布局手牌并移动到目标位置
        this.mHandCardList.ForEach( ( item ) =>
        {
            let GDCardUI : GDRecordCardUIPanel = item;
    ...
    private SetAllHandsCardsLayoutInfo()
    {
        ...
        {
            let Group : GDRecordHandCardGroup = this.mHandCardGrpList.Get( idx );
    ...
    {
        this.mRiverCardList.ForEach( ( item : GDRecordCardUIPanel)=>
        {
            if ( item == null )return;
    ...
    private InitSeats()
    {
        let Player : GamePlayer = RecordGuanDanScenes.Instance.GetRolePlayer( this.mBindRoleID );
        if ( null == Player ) return; 
        this.mGamePlayer = Player;
    }

    public GetBeforeGroupCarsNums( Grp : GDRecordHandCardGroup )
    {
        if ( null == Grp ) return 0;
        let Nums = 0;
        for ( let i = 1; i <= this.mHandCardGrpList.GetCount(); i++ )
        {
            let _grp : GDRecordHandCardGroup = this.mHandCardGrpList.Get( i );
            if ( Grp != _grp )
            {
    ...
    public ShowSendGiftCardEffect( MsgCardInfo )
    {
        ...
        let GDCardUI : GDRecordCardUIPanel = this.mGiftCardList[Idx];
        ...
        GDCardUI.SetGiftCardFlag( RecordGuanDanScenes.Instance.GetGiftCardIndex() , true);
    }
    ...
    public ShowRecvGiftCardEffect()
    {
        ...
        let GDGiftCardUI : GDRecordCardUIPanel = this.mGiftCardList[Idx];
        let VTo = this.GetGiftCardFromPos();
        let Action = cc.moveTo( 0.2 , VTo );
        GDGiftCardUI.node.stopAllActions()
        GDGiftCardUI.node.runAction(Action);
        GDGiftCardUI.SetGiftCardFlag(RecordGuanDanScenes.Instance.GetGiftCardIndex(),false);
    ...
    public ClearHandCards()
    {
        this.mHandCardList.ForEach( ( item ) =>
        {
            let Card : GDRecordCardUIPanel = item;
            if ( Card == null ) return;
            Card.node.destroy();
        });
    ...
    public ClearGroup()
    {
        ...
        {
            let Grp : GDRecordHandCardGroup = item;
            if ( Grp == null ) return;
            Grp.ClearCards();
        });
    ...
    public ResetHandCardGroup ( CardInfoList : LinkedList )
    {
        ...
        let NewGrp :GDRecordHandCardGroup = new  GDRecordHandCardGroup();
        ...
        for ( let i = 1; i <= CardInfoList.GetCount(); i++ )
        {
            ...
            {
                let  NewGroup : GDRecordHandCardGroup =  new GDRecordHandCardGroup();
            ...
            if ( null != GDUIPanel )
            {
                let LastGrp : GDRecordHandCardGroup  = this.mHandCardGrpList.Get( this.mHandCardGrpList.GetCount() );
                if ( null != LastGrp )
                {
    ...
    public FindGDCardUIByIndex( Index : number ) : GDRecordCardUIPanel
    {
        for ( let i = 1 ; i <= this.mHandCardList.GetCount(); i++ )
        {
            let HandCard :GDRecordCardUIPanel = this.mHandCardList.Get(i);
    ...
    public FindGDCardUIByCardValue( Card : number ) : GDRecordCardUIPanel
    {
        for ( let i = 1 ; i <= this.mHandCardList.GetCount(); i++ )
        {
            let HandCard :GDRecordCardUIPanel = this.mHandCardList.Get(i);
    ...
    //添加一个组
    private AddNewGroup( Grp : GDRecordHandCardGroup , InsertFirst : boolean)
    {
        if ( null == Grp ) return;
        Grp.GDSeat = this;
    ...
    //创建手牌(掼蛋中仅有主玩家需要手牌列表)
    private InitialAllHandCards( Deal : boolean )
    {
    ...
    {
        let Path = GameDefine.PrefabPath + "GuanDanRecordHandCardSmall";
    ...
    {
        let lGDCardInfoList = RecordGuanDanScenes.Instance.LastPlayerPutOutCars;
        ...
        let fun = cc.callFunc(()=>
        {
            if ( Count > 1 )
            {
                ...
                for ( let i = 1; i <= Count; i++ )
                {
                    ...
                    let Card : GDRecordCardUIPanel = this.mRiverCardList.Get(i);
    ...
    {
        this.mRiverIndex = 1;
        this.RiverCardList.ForEach( ( item : GDRecordCardUIPanel ) =>
        {
            if ( item == null) return;
            item.SetVisible( false );
            item.ResetPosition();
    ...
    private RiverCardSortSet( Count : number , CardInfoList : LinkedList )
    {
        ...
        else
        {
            for( let i = 1; i <= Count; i++ )
            {
                let RiverCard : GDRecordCardUIPanel = this.mRiverCardList.Get(i);
    ...
    private RiverCardHuLuSort( OutValue : number , Count : number, CardInfoList : LinkedList )
    {
        ...
        for ( let i = 1; i <= Count; i++ )
        {
            let CardLogicValue = RecordGuanDanScenes.Instance.GetGDCardLogicValue( CardInfoList.Get(i).Card );
        ...
        if ( ThreeCard < 3 )
        {
            for ( let i = 1; i <= Count; i++ )
            {
                if ( CardInfoList.Get(i).Card == RecordGuanDanScenes.Instance.CurSeries )
                {
                    UseIndex.Push( i );
                    let RiverCard : GDRecordCardUIPanel = this.mRiverCardList.Get(i);
                    if ( RiverCard == null ) continue;
    ...
    private RiverCardShunZiSort( OutValue : number , Count : number, CardInfoList : LinkedList )
    {
        ...
        {
            if ( item.Card == RecordGuanDanScenes.Instance.CurSeries )
            {
                RandMatchNums += 1;
            }
        } );
        ...
        for ( let i = CardInfoList.GetCount(); i >= 1; i-- )    //从小到大的顺序
        {
            ...
            if ( CardInfo.Card == RecordGuanDanScenes.Instance.CurSeries )
            {
        ...
        {
            let RiverCard : GDRecordCardUIPanel = this.mRiverCardList.Get(k);
            if ( RiverCard == null ) continue;
    ...
    private RiverCardGangBanSort( OutValue : number , Count : number, CardInfoList : LinkedList )
    {
        ...
        for ( let i = 1; i <= CardInfoList.GetCount(); i++ )
        {
            ...
            if ( Card == null ) continue;
            if ( Card.Card != RecordGuanDanScenes.Instance.CurSeries )
            {
    ...
    private GetGangBanSameValueNums( CardInfoList : LinkedList , Idx : number )
    {
        ...
        CardInfoList.ForEach( (item : GameCard)=>
        {
            if (item.Value == cv.Value && item.Value != RecordGuanDanScenes.Instance.CurSeries )
            {
                ret += 1;
            }
        } );
    ...
    //移除一个手牌组
    public RemoveGroup( Group : GDRecordHandCardGroup )
    {
    ...
    {
        this.mKingCardList.ForEach( (item : GDRecordCardUIPanel)=>
        {
            if ( item == null ) return;
            item.SetVisible( false );
        } );
        ...
        if ( RecordGuanDanScenes.Instance.IsResistKingRole( this.BindRoleID ) )
        {
            let KingRoleNums = RecordGuanDanScenes.Instance.GetResistKingRole().GetCount();
			let ShowNums = 0;
    ...
    public  get GiftCardList()      : GDRecordCardUIPanel[]   { return this.mGiftCardList;    }
    public  set CardLayout( Param : GuanDanCardLayout ) { this.mCardLayout = Param;     }
    public  set GiftCardList( Param : GDRecordCardUIPanel[] ) { this.mGiftCardList = Param;   }
}
