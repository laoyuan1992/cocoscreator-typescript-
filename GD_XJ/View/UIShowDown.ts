...
@ccclass
export default class UIShowDown extends UIBase
{
    @property(cc.Label)
    ...
    private ShowWinRole( ShowDownData : any )
    {
        for ( let i = 0; i < ShowDownData.win_role.length; i++ )
        {
            ...
            else
            {
                ...
                //显示提升等级
                this.TextMineTips.string = cc.js.formatStr ( "升%d级", GuanDanScenes.Instance.CurrShowDownData.series_inc );
            }
        }
    }
    ...
    private SelectWinBackGround( GDPlayer : GamePlayer )
    {
        if ( GDPlayer.GetTeam() == 1 )
        {
            ...
            //如果过A
            if ( GuanDanScenes.Instance.CurrShowDownData.one_game_over == 1 )
            {
    ...
    private OnClose()
    {
        if ( GuanDanScenes.Instance.CurrShowDownData.one_game_over == 1 ) return;
        GuanDanScenes.Instance.ClearPrivateGameData();
        GuanDanScenes.Instance.SendContinueGame();
        UIManager.DestroyUI( GameDefine.GameUIConfig.UI_SHOWDOWN.type );
    }
